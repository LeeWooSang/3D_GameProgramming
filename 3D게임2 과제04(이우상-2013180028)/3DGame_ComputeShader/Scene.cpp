...
void CScene::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	m_pTerrain = new CTerrain(pd3dDevice, pd3dCommandList, m_pd3dGraphicsRootSignature, _T("../Textures/GameObject/HeightMap.raw"), 257, 257, 257, 257, xmf3Scale, xmf4Color);
	...
	if (!m_pMissileShader)
	{
		...
		m_pMissileShader->BuildObjects(pd3dDevice, pd3dCommandList, NULL);
		...
		m_pMissileShader->SetExplosionParticleShader(m_pExplosionParticleShader);
	}
...
void CScene::ReleaseObjects()
{
	...
	if (m_pMissileShader)
	{
		m_pMissileShader->ReleaseShaderVariables();
		m_pMissileShader->ReleaseObjects();
		delete m_pMissileShader;
	}
...
void CScene::ReleaseUploadBuffers()
{
	...
	if (m_pMissileShader)
		m_pMissileShader->ReleaseUploadBuffers();
...
ID3D12RootSignature *CScene::CreateGraphicsRootSignature(ID3D12Device *pd3dDevice)
{
	...
	pd3dDescriptorRanges[12].BaseShaderRegister = 14; //t14: MissileTexture
	...
	pd3dDescriptorRanges[15].BaseShaderRegister = 16; // t16
...
ID3D12RootSignature *CScene::CreateComputeRootSignature(ID3D12Device *pd3dDevice)
{
	...
	pd3dDescriptorRanges[0].NumDescriptors = 1;
	pd3dDescriptorRanges[0].BaseShaderRegister = 17; //t17 
	pd3dDescriptorRanges[0].RegisterSpace = 0;
	pd3dDescriptorRanges[0].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

	pd3dDescriptorRanges[1].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_UAV;
	pd3dDescriptorRanges[1].NumDescriptors = 1;
	pd3dDescriptorRanges[1].BaseShaderRegister = 0; //u0: output
	pd3dDescriptorRanges[1].RegisterSpace = 0;
	pd3dDescriptorRanges[1].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;
...
bool CScene::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		switch (wParam)
		{
			...
			// 총알 발사 키
		case VK_CONTROL:
			if (m_pMissileShader && m_pFramePlayer && m_pFireParticleShader)
			{
				m_pMissileShader->SetParticleShader(m_pFireParticleShader);
				m_pMissileShader->SetFontShader(m_pFontShader);
				m_pMissileShader->SetTenthFontShader(m_pTenthFontShader);
				m_pMissileShader->OnProcessingKeyboardMessage(hWnd, nMessageID, wParam, lParam);
			}
			break;
...
void CScene::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera)
{
	...
	if (m_pFramePlayer)
	{
		...
		if (m_pMissileShader)
			m_pMissileShader->Render(pd3dCommandList, pCamera);
		...
		//if (m_pFontShader)
		//	m_pFontShader->Render(pd3dCommandList, pCamera,  ZERO);

		//if(m_pTenthFontShader)
		//	m_pTenthFontShader->Render(pd3dCommandList, pCamera, ZERO);
	}
}
...
void CScene::CheckObjectByObjectCollisions()
{
	...
	{
		// 총알과 몬스터와의 충돌체크
		if (m_ppFrameObjects && m_pMissileShader)
		{
			list<CGameObject*> pMissileList = m_pMissileShader->GetMissileList();
			for (auto iter = pMissileList.begin(); iter != pMissileList.end(); ++iter)
			{
				for (int i = 0; i < m_nFrameObjects; ++i)
				{
					if ((*iter)->m_xmOOBB.Intersects(m_ppFrameObjects[i]->m_xmOOBB))
					{
						...
						((CMissile*)(*iter))->SetCollision(true);
					}
				}
			}
		}
