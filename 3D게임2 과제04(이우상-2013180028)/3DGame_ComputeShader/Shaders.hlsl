...
Texture2D MissileTexture : register(t14);
float4 PSMissile(VS_TEXTURED_OUTPUT input) : SV_TARGET
{
	float4 cColor = MissileTexture.Sample(gWrapSamplerState, input.uv);
...
VS_TEXTURED_OUTPUT VSPostProcessing(uint nVertexID : SV_VertexID)
{
	...
	// 좌측 상단에 백미러가 위치하도록 스크린좌표계, uv 설정
	if (nVertexID == 0)
	{
...
{
	float4 cColor = gtxtScene.Sample(gWrapSamplerState, input.uv);
	return cColor;
}

VS_TEXTURED_OUTPUT VSFont(uint nVertexID : SV_VertexID)
{
	VS_TEXTURED_OUTPUT output;

	if (nVertexID == 0)
	{
		output.position = float4(0.75f, 1.f, 0.f, 1.f);
		output.uv = float2(1.f, 0.f);
	}
	else if (nVertexID == 1)
	{
		output.position = float4(0.75f, 0.75f, 0.f, 1.f);
		output.uv = float2(1.f, 1.f);
	}
	else if (nVertexID == 2)
	{
		output.position = float4(1.f, 0.75f, 0.f, 1.f);
		output.uv = float2(0.f, 1.f);
	}
	else if (nVertexID == 3)
	{
...
VS_TEXTURED_OUTPUT VSTenthFont(uint nVertexID : SV_VertexID)
{
	VS_TEXTURED_OUTPUT output;
...
float4 PSFont(VS_TEXTURED_OUTPUT input) : SV_TARGET
{
	float4 cColor = gtxtTexture.Sample(gClampSamplerState, input.uv);
	return(cColor);
}

VS_TEXTURED_OUTPUT VSMissileUI(uint nVertexID : SV_VertexID)
{
	VS_TEXTURED_OUTPUT output;
...
Texture2D gtxtInput : register(t17);
...
{
	// 각 배열의 인덱스에 넣는다.
	float weights[11] = { 0.05f, 0.05f, 0.1f, 0.1f, 0.2f, 0.1f, 0.1f, 0.1f, 0.1f, 0.05f, 0.05f };

	if (groupThreadID.x < gMaxBlurRadius)
	{
		// 이미지 왼쪽 가장자리 바깥의 표본을 왼쪽 가장자리 표본으로 한정한다.
		int x = max(dispatchThreadID.x - gMaxBlurRadius, 0);
		gCache[groupThreadID.x] = gtxtInput[int2(x, dispatchThreadID.y)];
	}
	if (groupThreadID.x >= N - gMaxBlurRadius)
	{
		// 이미지 가장자리 바깥의 표본을 오른쪽 가장자리 표본으로 한정한다.
		int x = min(dispatchThreadID.x + gMaxBlurRadius, gtxtInput.Length.x - 1);
		gCache[groupThreadID.x + 2 * gMaxBlurRadius] = gtxtInput[int2(x, dispatchThreadID.y)];
	}
	// 이미지 가장자리 바깥의 표본을 이미지 가장자리 표본으로 한정한다.
	gCache[groupThreadID.x + gMaxBlurRadius] = gtxtInput[min(dispatchThreadID.xy, gtxtInput.Length.xy - 1)];
	...
	{
		int k = groupThreadID.x + gMaxBlurRadius + i;
	...
	int3 dispatchThreadID : SV_DispatchThreadID)
{
	...
	if (groupThreadID.y < gMaxBlurRadius)
	{
		int y = max(dispatchThreadID.y - gMaxBlurRadius, 0);
		gCache[groupThreadID.y] = gtxtInput[int2(dispatchThreadID.x, y)];

	}
	if (groupThreadID.y >= N - gMaxBlurRadius)
	{
		int y = min(dispatchThreadID.y + gMaxBlurRadius, gtxtInput.Length.y - 1);
		gCache[groupThreadID.y + 2 * gMaxBlurRadius] = gtxtInput[int2(dispatchThreadID.x, y)];

	}
	gCache[groupThreadID.y + gMaxBlurRadius] = gtxtInput[min(dispatchThreadID.xy, gtxtInput.Length.xy - 1)];
	...
	for (int i = -gMaxBlurRadius; i <= gMaxBlurRadius; ++i)
	{
		int k = groupThreadID.y + gMaxBlurRadius + i;
