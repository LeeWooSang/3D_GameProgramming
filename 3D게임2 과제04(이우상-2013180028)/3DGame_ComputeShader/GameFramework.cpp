...
void CGameFramework::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
		...
		{
		case 'F':
			if(m_pBlurringShader->GetOnBlurring() == false)
				m_pBlurringShader->SetOnBlurring(true);
			else
				m_pBlurringShader->SetOnBlurring(false);
			break;

		case 'S':
			m_pUIManager->GetUIShader()->DeleteUI(GAMESTART);
			g_GameState = GAMERUN;
			break;

		case VK_TAB:
			m_pUIManager->GetUIShader()->Initialize(GAMEMENU);
			g_GameState = GAMEMENU;
			break;
			
		case 'P':
			if (g_GameState == GAMEMENU)
			{
				m_pUIManager->GetUIShader()->DeleteUI(GAMEMENU);
				g_GameState = GAMERUN;
			}			
			break;
		...
		case 'H':
			...
			{
				m_pUIManager->GetUIShader()->DeleteUI(GAMEMENU);
				m_pUIManager->GetUIShader()->Initialize(GAMEHELP);
				g_GameState = GAMEHELP;				
			}
			break;
		...
		case VK_ESCAPE:
			...
			{
				m_pUIManager->GetUIShader()->DeleteUI(GAMEMENU);
				m_pUIManager->GetUIShader()->Initialize(GAMEOVER);
				g_GameState = GAMEOVER;
			}
			else if (g_GameState == GAMEHELP)
			{
				m_pUIManager->GetUIShader()->DeleteUI(GAMEHELP);
				m_pUIManager->GetUIShader()->Initialize(GAMEMENU);
				g_GameState = GAMEMENU;
			}
			break;
		...
		case VK_RETURN:
			break;
		case VK_F1:
			m_pUIManager->SetCameraMode(FIRST_PERSON_CAMERA);
			m_pUIManager->GetUIShader()->Initialize(FIRST_PERSON);
			...
			break;
		case VK_F2:
			m_pUIManager->SetCameraMode(SPACESHIP_CAMERA);
			m_pUIManager->GetUIShader()->DeleteUI(FIRST_PERSON);
			m_pCamera = m_pFramePlayer->ChangeCamera(SPACESHIP_CAMERA, m_GameTimer.GetTimeElapsed());
			break;
		case VK_F3:
			m_pUIManager->SetCameraMode(THIRD_PERSON_CAMERA);
			m_pUIManager->GetUIShader()->DeleteUI(FIRST_PERSON);
...
void CGameFramework::BuildObjects()
{
	...
	m_pUIManager->SetScene(m_pScene);
	m_pUIManager->BuildObjects(m_pd3dDevice, m_pd3dCommandList);
	...
	m_pPostProcessingShader->BuildObjects(m_pd3dDevice, m_pd3dCommandList, m_pTextureForPostProcessing);

	m_pBlurringShader = new CBlurringShader;
	m_pBlurringShader->CreateShader(m_pd3dDevice, m_pScene->GetComputeRootSignature());
	m_pBlurringShader->BuildObjects(m_pd3dDevice, m_pd3dCommandList, m_pTextureForBlurring);
...
void CGameFramework::ReleaseObjects()
{
	...
	if (m_pFramePlayer)
	{
		m_pFramePlayer->ReleaseUploadBuffers();
...
void CGameFramework::AnimateObjects()
{
	...
	if (m_pFramePlayer)
		m_pFramePlayer->Animate(m_GameTimer.GetTimeElapsed(), NULL);	
}
...
void CGameFramework::FrameAdvance()
{
	...
	// 전에는 읽기 상태였는데 그리는걸로 상태를 바꾸겠다. 스크린에 안그리는 렌더타겟버퍼 0번을.
	for (int i = 0; i < m_nOffScreenRenderTargetBuffers; i++)
	...
	hResult = m_pd3dCommandList->Close();
	ID3D12CommandList *ppd3dPostCommandLists[] = { m_pd3dCommandList };
	...
	///////////////////////////////////////////////////////////////////////////
	// 자 이제 후면버퍼의 상태를 보여주는 상태에서 렌더타겟에 그리는상태로 바꾼다고 알려줌.
	...
	if (m_pBlurringShader->GetOnBlurring() == true)
		m_pBlurringShader->Render(m_pd3dCommandList, m_pCamera, 0, m_ppd3dSwapChainBackBuffers[m_nSwapChainBufferIndex]);

	if (m_pUIManager)
		m_pUIManager->Render(m_pd3dCommandList);
...
{
	m_pTextureForPostProcessing = new CTexture(m_nOffScreenRenderTargetBuffers, RESOURCE_TEXTURE2D_ARRAY, 0);
	m_pTextureForBlurring = new CTexture(m_nBlurScreenRenderTargetBuffers, RESOURCE_TEXTURE2D, 0);
	...
	{
		m_ppd3dOffScreenRenderTargetBuffers[i] = m_pTextureForPostProcessing->CreateTexture(m_pd3dDevice, m_pd3dCommandList, m_nWndClientWidth, m_nWndClientHeight, DXGI_FORMAT_R8G8B8A8_UNORM, D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET, D3D12_RESOURCE_STATE_GENERIC_READ, &d3dClearValue, i);
		m_ppd3dOffScreenRenderTargetBuffers[i]->AddRef();
	}

	for (UINT i = 0; i < m_nBlurScreenRenderTargetBuffers; i++)
	{
		m_ppd3dScreenBlur[i] = m_pTextureForBlurring->CreateTexture(m_pd3dDevice, m_pd3dCommandList, m_nWndClientWidth, m_nWndClientHeight, DXGI_FORMAT_R8G8B8A8_UNORM, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_COMMON, nullptr, i);
	...
	for (UINT i = 0; i < m_nOffScreenRenderTargetBuffers; i++)
	{
		...
		m_pd3dDevice->CreateRenderTargetView(m_pTextureForPostProcessing->GetTexture(i), &d3dRenderTargetViewDesc, m_pd3dOffScreenRenderTargetBufferCPUHandles[i]);
		d3dRtvCPUDescriptorHandle.ptr += m_nRtvDescriptorIncrementSize;
	}
}
