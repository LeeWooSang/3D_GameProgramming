...
void CShader::CreateCbvSrvUavDescriptorHeaps(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, int nConstantBufferViews, int nShaderResourceViews, int nUnorderedAccessViews)
{
	D3D12_DESCRIPTOR_HEAP_DESC d3dDescriptorHeapDesc;
...
void CShader::CreateUnorderedAccessViews(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, CTexture *pTexture, UINT nRootParameterStartIndex, bool bAutoIncrement)
{
	...
	for (int i = 0; i < nTextures; i++)
	{
		...
		D3D12_RESOURCE_DESC d3dResourceDesc = pResource->GetDesc();
		D3D12_UNORDERED_ACCESS_VIEW_DESC d3dUnorderedResourceDesc = {};
		d3dUnorderedResourceDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
		d3dUnorderedResourceDesc.ViewDimension = D3D12_UAV_DIMENSION_TEXTURE2D;
		d3dUnorderedResourceDesc.Texture2D.MipSlice = 0;

		pd3dDevice->CreateUnorderedAccessView(pResource, nullptr, &d3dUnorderedResourceDesc, m_d3dUavCPUDescriptorStartHandle);
		m_d3dUavCPUDescriptorStartHandle.ptr += ::gnCbvSrvDescriptorIncrementSize;

		// 해당 텍스처를 루트시그니처에 셋
		pTexture->SetRootArgumentUav(i, (bAutoIncrement) ? (nRootParameterStartIndex + i) : nRootParameterStartIndex, m_d3dUavGPUDescriptorStartHandle);
...
void CShader::CreateShaderResourceViews(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, CFrameTexture *pTexture, UINT nRootParameterStartIndex, bool bAutoIncrement)
{
	int nTextures = pTexture->GetTextures();
	int nTextureType = pTexture->GetTextureType();
	for (int i = 0; i < nTextures; i++)
