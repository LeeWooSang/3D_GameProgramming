...
void CFontShader::CreateShader(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature)
{
	m_nPipelineStates = 1;
...
void CFontShader::ReleaseShaderVariables()
{
	CTexturedShader::ReleaseShaderVariables();
}

void CFontShader::ReleaseUploadBuffers()
{
	for(auto iter = m_FontMap.begin(); iter != m_FontMap.end(); ++iter)
		(*iter).second->ReleaseUploadBuffers();
}

bool CFontShader::Initialize(int key)
{
	m_num = key;
	m_pFont = new CFont;
...
void CFontShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
	if (!m_pFontMesh)
...
void CFontShader::ReleaseObjects()
{
	m_pFontMesh->Release();
	DeleteUI(-1);
}

void CFontShader::DeleteUI(int key)
{
	if (key != -1)
	{
...
void CFontShader::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera, int key)
{
