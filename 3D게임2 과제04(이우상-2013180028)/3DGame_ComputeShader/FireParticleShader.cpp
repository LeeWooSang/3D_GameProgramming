...
bool CFireParticleShader::Initialize(CGameObject* pMissile, int id)
{
	if (pMissile)
	{
		...
		pParticle->SetPosition(pMissile->GetPosition());
...
void CFireParticleShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, void *pContext)
{
	...
	m_pFireParticleTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"../Textures/GameObject/FireParticle.dds", 0);
	// 불꽃 파티클 재질 생성
