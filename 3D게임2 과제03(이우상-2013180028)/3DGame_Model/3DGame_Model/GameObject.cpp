...
{
	if (!strncmp(m_pstrFrameName, "rotor", 5))
	{
		XMFLOAT3 xmf3RotationAxis = Vector3::Normalize(XMFLOAT3(m_xmf4x4Transform._21, m_xmf4x4Transform._22, m_xmf4x4Transform._23));
...
void CGameObject::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera)
{
	...
	if (m_nMaterials > 0)
	{
		for (int i = 0; i < m_nMaterials; i++)
		{
			...
			UpdateShaderVariable(pd3dCommandList, m_ppMaterials[i]);
...
void CGameObject::UpdateShaderVariable(ID3D12GraphicsCommandList *pd3dCommandList, CMaterial *pMaterial)
{
	...
	{
		pd3dCommandList->SetGraphicsRoot32BitConstants(2, 4, &(pMaterial->m_pMaterialColors->m_xmf4Ambient), 16);
		pd3dCommandList->SetGraphicsRoot32BitConstants(2, 4, &(pMaterial->m_pMaterialColors->m_xmf4Diffuse), 20);
		pd3dCommandList->SetGraphicsRoot32BitConstants(2, 4, &(pMaterial->m_pMaterialColors->m_xmf4Specular), 24);
		pd3dCommandList->SetGraphicsRoot32BitConstants(2, 4, &(pMaterial->m_pMaterialColors->m_xmf4Emissive), 28);
	}
}
...
CMeshLoadInfo *CGameObject::LoadMeshInfoFromFile(FILE *pInFile)
{
	...
	for ( ; ; )
	{
		...
		{
			nReads = (UINT)::fread(&(pMeshInfo->m_xmf3AABBCenter), sizeof(float), 6, pInFile);
		}
		else if (!strcmp(pstrToken, "<Positions>:"))
		{
			...
			if (nPositions > 0)
			{
				...
				// 포지션 정보를 한번에 읽어서 빌드하는데 속도를 올려준다.
...
CGameObject *CGameObject::LoadGeometryFromFile(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature, char *pstrFileName)
{
	...
	CGameObject *pGameObject = CGameObject::LoadFrameHierarchyFromFile(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, pInFile);
